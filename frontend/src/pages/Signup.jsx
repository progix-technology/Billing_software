    ...
    <div className="min-h-screen bg-cream-100 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-xl p-8 relative z-10 transition-colors duration-350">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-sm text-slate-500 mt-1">Register new user account inside system</p>
        </div>

        {error && (
          <div className="mb-5 p-4 bg-red-50 border border-red-200 rounded-xl flex items-center gap-3 text-sm text-red-650 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-4 text-slate-950 focus:outline-none focus:border-primary-500 focus:bg-white transition-all duration-200 capitalize"
            >
              <option value="admin">Admin (Full Access)</option>
              <option value="manager">Manager (Inventory/Ledger)</option>
              <option value="staff">Staff/Cashier (POS Checkout)</option>
            </select>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-primary-600 hover:bg-primary-500 active:bg-primary-700 text-white font-semibold py-3 rounded-xl shadow-md transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center mt-2"
          >
            {loading ? (
