...
import { Truck, Plus, Edit2, Trash2, X, Search, FileSpreadsheet, Eye, IndianRupee } from 'lucide-react';
...
const Suppliers = () => {
  ...
  const openLedgerModal = async (s) => {
    ...
    setPaymentRemarks('');
    try {
      const res = await api.get(`/api/suppliers/${s._id}/ledger`);
      if (res.data.success) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (formMode === 'CREATE') {
        ...
        const res = await api.put(`/api/suppliers/${editId}`, payload);
        if (res.data.success) {
          setShowFormModal(false);
          fetchSuppliers();
        }
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        const ledgerRes = await api.get(`/api/suppliers/${ledgerSupplier._id}/ledger`);
      ...
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 transition-colors duration-300">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            ...
            Supplier Registry & Purchases
          </h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Track suppliers, outstanding balances, inward stock logistics, and payment clearing history.
          </p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors duration-300">
        ...
        {loading ? (
          <div className="p-12 text-center text-slate-500">
            ...
            <p className="mt-3 text-sm">Loading supplier directory...</p>
          </div>
        ) : suppliers.length === 0 ? (
          <div className="p-12 text-center text-slate-500">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-950/50 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                    ...
                    <th className="py-4 px-6">We Owe Balance</th>
                    <th className="py-4 px-6 text-right">Ledgers & Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 text-sm">
                  {suppliers.map((s) => {
                    const weOwe = s.outstandingBalance > 0;
                    return (
                      <tr key={s._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 text-slate-700 dark:text-slate-300">
                        <td className="py-4 px-6 font-semibold text-slate-900 dark:text-white">
                          <div>
                            {s.name}
                            {s.contactPerson && <p className="text-xs font-normal text-slate-400">Contact: {s.contactPerson}</p>}
                          </div>
                        </td>
                        ...
                        <td className="py-4 px-6">
                          <span className={`font-bold rounded-full text-xs px-2 py-0.5 ${
                            ...
                            {weOwe ? `₹${s.outstandingBalance.toFixed(2)}` : 'Settle'}
                          </span>
                        </td>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden transition-colors duration-300">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/40">
              <h3 className="font-bold text-slate-900 dark:text-white">
                {formMode === 'CREATE' ? 'Add New Supplier Profile' : 'Edit Supplier Details'}
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Royal Distributions"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. info@royal.com"
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl py-2 px-3 focus:outline-none focus:border-primary-500 text-sm text-slate-900 dark:text-white"
                  />
                </div>
                ...
                {formMode === 'CREATE' && (
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Amount we owe them"
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl py-2 px-3 focus:outline-none focus:border-primary-500 text-sm text-slate-900 dark:text-white"
                    />
                  </div>
                )}
            ...
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/40">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Transactions with {ledgerSupplier?.name} | Phone: {ledgerSupplier?.phone}
                </p>
              </div>
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl">
                  <span className="text-xs text-slate-400">Total We Owe Balance</span>
                  <p className="text-2xl font-black text-amber-500 mt-1">
                    ₹{ledgerSupplier?.outstandingBalance.toFixed(2)}
                  </p>
                </div>
                
                <div className="md:col-span-2 flex items-end justify-end">
                  {!showPaymentForm && ledgerSupplier?.outstandingBalance > 0 && (
                    <button
                    ...
                    >
                      <IndianRupee className="h-4 w-4" /> Record Outbound Payment
                    </button>
                  )}
                </div>
              </div>

              {/* Record outbound payment form */}
              {showPaymentForm && (
                <form onSubmit={handlePaymentSubmit} className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl space-y-3 animate-fade-in">
                  <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-2">
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-300">Enter Payment Details (Outbound)</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-slate-400 mb-1">Remarks / Reference</label>
                      <input
                        type="text"
                        value={paymentRemarks}
                        onChange={(e) => setPaymentRemarks(e.target.value)}
                        placeholder="e.g. UPI Ref #7463"
                        className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-2 text-sm"
                      />
                    </div>
                  </div>
              ...
              {ledgerLoading ? (
                <div className="p-8 text-center text-slate-500">
                  ...
                  <p className="mt-2 text-xs">Compiling statement transaction files...</p>
                </div>
              ) : ledgerList.length === 0 ? (
                <p className="text-center py-8 text-sm text-slate-500">No transactions recorded yet</p>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-slate-950/50 font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                        ...
                        <th className="py-3 px-4 text-right">Debit (We Paid)</th>
                        <th className="py-3 px-4 text-right">Credit (We Bought)</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                      {ledgerList.map((entry) => (
                        <tr key={entry._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 text-slate-600 dark:text-slate-300">
                          ...
                          <td className="py-3 px-4 text-right font-bold text-emerald-500">{entry.debit > 0 ? `₹${entry.debit.toFixed(2)}` : '-'}</td>
                          <td className="py-3 px-4 text-right font-bold text-amber-500">{entry.credit > 0 ? `₹${entry.credit.toFixed(2)}` : '-'}</td>
